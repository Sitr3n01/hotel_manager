...
function getChangedFiles() {
  const warnings = [];
  if (!isInsideGitWorkTree(REPO_ROOT)) {
    warnings.push({
      severity: "info",
...
function collectFileMetrics(config) {
  ...
  const gitPrefix = getGitPrefix(REPO_ROOT);
  const untrackedFiles = getUntrackedFiles(gitPrefix);
  const changedFiles = diffResult.files
    ? unique([
        ...diffResult.files.map((f) => normalizeGitPath(f, gitPrefix)),
        ...untrackedFiles,
      ]).filter((f) => fileLineCounts[f] !== undefined)
    : null;
  ...
  if (isInsideGitWorkTree(REPO_ROOT)) {
    ...
    if (refs && refs.includes("...")) {
      ...
      if (res.ok) {
        addedFiles = res.stdout.split(/\r?\n/).map((s) => s.trim()).map((f) => normalizeGitPath(f, gitPrefix)).filter(Boolean);
      }
    }
  }
  addedFiles = unique([...addedFiles, ...untrackedFiles]).filter((f) => fileLineCounts[f] !== undefined);
...
function normalizeGitPath(file, gitPrefix) {
  const normalized = normalizePath(file);
  if (gitPrefix && normalized.startsWith(gitPrefix)) {
    return normalized.slice(gitPrefix.length);
  }
  return normalized;
}

function getUntrackedFiles(gitPrefix) {
  if (!isInsideGitWorkTree(REPO_ROOT)) return [];
  const res = runCommandSafe("git", ["ls-files", "--others", "--exclude-standard"]);
  if (!res.ok) return [];
  return res.stdout
    .split(/\r?\n/)
    .map((s) => s.trim())
    .filter(Boolean)
    .map((f) => normalizeGitPath(f, gitPrefix));
}
