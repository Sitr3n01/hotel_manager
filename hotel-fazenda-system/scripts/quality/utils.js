...
function isInsideGitWorkTree(cwd = REPO_ROOT) {
  const res = runCommandSafe("git", ["rev-parse", "--is-inside-work-tree"], { cwd });
  return res.ok && res.stdout.trim() === "true";
}

function getGitPrefix(cwd = REPO_ROOT) {
  const res = runCommandSafe("git", ["rev-parse", "--show-prefix"], { cwd });
  if (!res.ok) return "";
  return normalizePath(res.stdout.trim());
}
